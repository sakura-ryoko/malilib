...
//        LOGGER.warn("toInventory(): sizeAdj [{}] -> inv size [{}]", sizeAdj, inv.getContainerSize());
     ...
     * @return -
     */
    public Pair<Inventory, Inventory> splitInventory() throws IllegalStateException
    {
        if (this.size() % 2 != 0)
        {
        ...
        Inventory inv1 = new SimpleInventory(halfSize);
        Inventory inv2 = new SimpleInventory(halfSize);
...
//        LOGGER.warn("splitInventory(): sizeAdj [{}] -> halfSize [{}]", sizeAdj, halfSize);
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            if (slot < halfSize)
            {
                inv1.setStack(slot, stack.copy());
            }
            else
            {
                inv2.setStack((slot - halfSize), stack.copy());
            }
        }
...
//        newInv.dumpInv();
    ...
    public static class EntrySlot
    {
        ...
        public NbtCompound toNbt(@Nonnull DynamicRegistryManager registry)
        {
            ...
            try
            {
                data = (NbtCompound) ItemStack.CODEC.encodeStart(ops, this.stack).getPartialOrThrow();
            }
            catch (Exception e)
            {
                ...
                builder.add(DataComponentTypes.LORE, lore);
                ItemStack fallback = new ItemStack(Registries.ITEM.getEntry(Items.BARRIER), 1, builder.build());
                data = (NbtCompound) ItemStack.CODEC.encodeStart(ops, fallback).getPartialOrThrow();
            }
