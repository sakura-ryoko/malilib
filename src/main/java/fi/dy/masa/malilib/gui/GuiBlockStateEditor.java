...
public class GuiBlockStateEditor extends GuiDialogSplitBase
{
	...
	private <T extends Comparable<T>> void refreshBlockState()
	{
		...
		this.tempBlockId = Registries.BLOCK.getId(this.block);
		...
		if (!this.props.isEmpty())
		{
			for (Property<?> prop : this.props)
			{
				...
				T value = this.blockState.get(entry);
...
//		final int adjWidth = MathUtils.max(MathUtils.max((this.maxLength * 2) + 6, (this.titleWidth * 2) + 6), 256) + 8;
		...
		this.centerOnScreen();
		this.init(MinecraftClient.getInstance(), this.dialogTotalWidth, this.dialogTotalHeight);
	}

	private <T extends Comparable<T>> void trySaveBlockState() throws Exception
	{
		Optional<Block> opt = Registries.BLOCK.getOptionalValue(this.tempBlockId);
		...
		NbtCompound tagProps = new NbtCompound();

		// TODO -- 26.3 (name)
		tag.putString(BLOCK_NAME, Registries.BLOCK.getId(this.block).toString());

		Map<Property<?>, Comparable<?>> values = this.blockState.getEntries();
		...
		if (!values.isEmpty())
		{
			for (Property<?> prop : this.props)
			{
				...
				{
					Optional<T> newVal = entry.parse(tmp);
	...
	private void addBlockName(int x, int y)
	{
		...
		this.textFieldBlockName = new GuiTextFieldGeneric(x, y, this.maxLength + 2, this.elementHeight, this.textRenderer);
		this.textFieldBlockName.setText(this.tempBlockId.toString());
	...
	{
		final String name = prop.getName();
		final List<T> validValues = prop.getValues();
		...
		this.addLabel(x, y, this.getStringWidth(str), this.elementHeight, COLOR_WHITE, str);
		y += this.elementHeight + 2;
		this.textFieldBlockProps.put(name, new GuiTextFieldGeneric(x, y, this.maxLength + 2, this.elementHeight, this.textRenderer));
		...
		try
		{
			value = this.blockState.get(prop);
		}
		...
		this.textFieldBlockProps.get(name).setText(valStr);
	...
	public void render(@NotNull DrawContext ctx, int mouseX, int mouseY, float partialTicks)
	{
		...
		this.drawBlockStateInGui(ctx, mouseX, mouseY);
	}

	private void drawBlockStateInGui(DrawContext ctx, int mouseX, int mouseY)
	{
			...
			implements ITextFieldListener<GuiTextFieldGeneric>
	{
		...
		{
			Identifier id = Identifier.tryParse(textField.getText());

			if (id != null && Registries.BLOCK.getEntry(id).isPresent())
			{
			...
			implements ITextFieldListener<GuiTextFieldGeneric>
	{
		...
		public boolean onTextChange(GuiTextFieldGeneric textField)
		{
			...
			{
				if (prop.getName().equals(this.name()) && prop.parse(val).isPresent())
				{
