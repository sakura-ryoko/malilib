...
//		ctx.itemDecorations(ctx.fontRenderer(), stack, 0, 0);
	...
	private ItemStack createItemStackForRendering()
	{
		...
		final RegistryEntry.Reference<Item> ref = ((IMixinItem) item).malilib_getBuiltInRegistryHolder();

		if (ref.hasKeyAndValue())
		{
			return new ItemStack(ref.value(), 1);
		}
		...
		return new ItemStack(item, 1, new MergedComponentMap(ComponentMap.EMPTY));
	}
}
