...
public class TextPlateRenderState extends AbstractTextPlateRenderState
{
	...
	{
		return this.seeThrough ? MaLiLibPipelines.TEXT_PLATE_MASA_NO_DEPTH : MaLiLibPipelines.TEXT_PLATE_MASA;
	}
	...
	public void update(VertexConsumer consumer)
	{
		...
		{
			maxLineLen = MathUtils.max(maxLineLen, font.getWidth(line));
		}

		this.strLenHalf = maxLineLen / 2;
		int textHeight = font.fontHeight * this.text.size() - 1;
		int bga = ((bgColor >>> 24) & 0xFF);
		int bgr = ((bgColor >>> 16) & 0xFF);
		int bgg = ((bgColor >>> 8) & 0xFF);
		int bgb = (bgColor & 0xFF);

		consumer.vertex((float) (-this.strLenHalf - 1), (float) -1, 0.0F).color(bgr, bgg, bgb, bga);
		consumer.vertex((float) (-this.strLenHalf - 1), (float) textHeight, 0.0F).color(bgr, bgg, bgb, bga);
		consumer.vertex((float) this.strLenHalf, (float) textHeight, 0.0F).color(bgr, bgg, bgb, bga);
		consumer.vertex((float) this.strLenHalf, (float) -1, 0.0F).color(bgr, bgg, bgb, bga);
	}
}
