...
public class SelectionBoxRenderer implements IOnDemandRenderer<AbstractSelectionBoxRenderState>
{
	...
	private void setupRenderContext()
	{
		if (this.hasCurrentData())
		{
			...
			{
				this.renderAreaOutlinesNoCorners = new RenderContext(() -> MaLiLibReference.MOD_ID + ":selection_box/area_outlines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
			}

			if (this.renderAreaSides == null)
			{
				this.renderAreaSides = new RenderContext(() -> MaLiLibReference.MOD_ID + ":selection_box/area_sides", MaLiLibPipelines.POSITION_COLOR_TRANSLUCENT_LEQUAL_DEPTH);
			}

			if (this.renderBlockOutlines1 == null)
			{
				this.renderBlockOutlines1 = new RenderContext(() -> MaLiLibReference.MOD_ID + ":selection_box/block_outlines1", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
			}

			if (this.renderBlockOutlines2 == null)
			{
				this.renderBlockOutlines2 = new RenderContext(() -> MaLiLibReference.MOD_ID + ":selection_box/block_outlines2", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
			}

			if (this.renderBlockOutlinesOverlapping == null)
			{
				this.renderBlockOutlinesOverlapping = new RenderContext(() -> MaLiLibReference.MOD_ID + ":selection_box/block_outlines_overlap", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
			}
	...
	@Override
	public @Nullable AbstractSelectionBoxRenderState updatePre(Camera camera, RenderTickCounter tracker, Profiler profiler)
	{
		if (this.hasData())
		{
			synchronized (this.pending)
	...
	@Override
	public @Nullable AbstractSelectionBoxRenderState drawPre(Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		...
		try
		{
			...
			this.batchAndDraw(this.renderAreaOutlinesNoCorners,
			                  this.currentEntries.stream().map(BoxEntry::areaOutlineNoCorners).filter(s -> !s.isEmpty()).toList(), true);
			...
			this.batchAndDraw(this.renderAreaSides, insideSides, false);

			List<AreaSidesRenderState> outsideSides = sides.stream().filter(s -> !s.isCameraInside()).toList();
			this.batchAndDraw(this.renderAreaSides, outsideSides, false);

			// Block Outlines 1 & 2
			this.batchAndDraw(this.renderBlockOutlines1,
			                  this.currentEntries.stream().map(BoxEntry::blockOutlines).filter(s -> !s.isEmpty()).toList(), true);

			this.batchAndDraw(this.renderBlockOutlines2,
			                  this.currentEntries.stream().map(BoxEntry::blockOutlines2).filter(s -> !s.isEmpty()).toList(), true);

			// Overlapping Block Outlines
			this.batchAndDraw(this.renderBlockOutlinesOverlapping,
			                  this.currentEntries.stream().map(BoxEntry::blockOutlineOverlapping).filter(s -> !s.isEmpty()).toList(), true);
		}
	...
	private void batchAndDraw(RenderContext ctx, List<? extends AbstractSelectionBoxRenderState> states, boolean setLineWidth)
	{
		if (states.isEmpty()) { return; }
		AbstractSelectionBoxRenderState firstState = states.getFirst();
		BufferBuilder builder = ctx.start(() -> MaLiLibReference.MOD_ID+":selection_box/batch_draw", firstState.pipeline());
		...
		if (setLineWidth)
		{
			ctx.lineWidth(firstState.lineWidth());
		}

		try (BuiltBuffer meshData = builder.endNullable())
		{
			if (meshData != null)
			{
				ctx.draw(meshData, firstState.shouldResort(), setLineWidth);
			}
			ctx.reset();
		}
		catch (Exception e)
