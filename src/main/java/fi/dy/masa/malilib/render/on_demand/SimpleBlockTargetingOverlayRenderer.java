	...
	private void checkConfigAndTarget(MinecraftClient mc)
	{
			...
			this.config.getBooleanValue())
		{
			...
			if (this.keybind != null)
			{
				...
				{
					this.scheduleTarget(entity.getHorizontalFacing(), hitResult.getBlockPos(), hitResult.getSide());
				}
				else { this.reset(); }
			}
			...
			if (this.renderLines != null && !this.renderLines.isUploaded())
			{
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						...
						this.renderLines.lineWidth(this.currentEntry.linesState().lineWidth());
