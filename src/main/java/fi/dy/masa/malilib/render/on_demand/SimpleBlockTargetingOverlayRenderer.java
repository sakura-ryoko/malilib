...
public class SimpleBlockTargetingOverlayRenderer implements IOnDemandRenderer<AbstractBlockTargetingOverlayRenderState>
{
	...
	private void checkConfigAndTarget(MinecraftClient mc)
	{
			...
			this.config.getBooleanValue())
		{
			...
			if (this.keybind != null)
			{
				...
				{
					this.scheduleTarget(entity.getFacing(), hitResult.getBlockPos(), hitResult.getSide());
				}
				else { this.reset(); }
			}
	...
	private void scheduleTarget(net.minecraft.util.math.Direction facing, net.minecraft.util.math.BlockPos pos, net.minecraft.util.math.Direction side)
	{
	...
	private void setupRenderContext()
	{
		if (this.currentEntry != null)
		{
			...
			{
				this.renderQuads = new RenderContext(() -> MaLiLibReference.MOD_ID+":simple_block_targeting_overlay/quads", this.currentEntry.quadsState().pipeline());
			}
			if (this.renderLines == null)
			{
				this.renderLines = new RenderContext(() -> MaLiLibReference.MOD_ID+":simple_block_targeting_overlay/lines", this.currentEntry.linesState().pipeline());
			}
	...
	private void uploadBuffers()
	{
		if (this.currentEntry != null)
		{
			...
			{
				BufferBuilder buffer = this.renderQuads.start(() -> MaLiLibReference.MOD_ID+":block_targeting_overlay/quads", this.currentEntry.quadsState().pipeline());
				this.currentEntry.quadsState().update(buffer);

				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
			...
			{
				BufferBuilder buffer = this.renderLines.start(() -> MaLiLibReference.MOD_ID+":block_targeting_overlay/lines", this.currentEntry.linesState().pipeline());
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						this.renderLines.lineWidth(this.lineWidth);
	...
	@Override
	public @Nullable AbstractBlockTargetingOverlayRenderState updatePre(Camera camera, RenderTickCounter tracker, Profiler profiler)
	{
		if (this.hasData() && this.dirty)
		{
			...
			{
				this.lastTarget = this.currentTarget;
				this.currentEntry = this.buildEntry(Vec3d.of(camera.getCameraPos()), this.currentTarget.facing(), this.currentTarget.pos(), this.currentTarget.side());
	...
	@Override
	public @Nullable AbstractBlockTargetingOverlayRenderState drawPre(Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		if (this.hasEntry())
		{
