...
public class WallOverlayRenderer implements IOnDemandRenderer<AbstractWallOverlayRenderState>
{
	...
	private void setupRenderContext()
	{
		if (this.hasCurrentData())
		{
			...
			{
				this.renderQuads = new RenderContext(() -> MaLiLibReference.MOD_ID + ":walls/quads", MaLiLibPipelines.MINIHUD_SHAPE_OFFSET_NO_CULL);
			}

			if (this.renderOutlines == null)
			{
				this.renderOutlines = new RenderContext(() -> MaLiLibReference.MOD_ID + ":walls/outlines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_LEQUAL_DEPTH);
			}
	...
	@Override
	public @Nullable AbstractWallOverlayRenderState updatePre(Camera camera, RenderTickCounter tracker, Profiler profiler)
	{
		if (this.hasData())
		{
			synchronized (this.pending)
	...
	@Override
	public @Nullable AbstractWallOverlayRenderState drawPre(Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		...
		{
			// Wall Quads
			this.batchAndDraw(this.renderQuads, this.currentEntries.stream().map(WallEntry::quads).toList(), false);

			// Wall Outlines
			this.batchAndDraw(this.renderOutlines, this.currentEntries.stream().map(WallEntry::outlines).toList(), true);
		}
	...
	private void batchAndDraw(RenderContext ctx, List<? extends AbstractWallOverlayRenderState> states, boolean setLineWidth)
	{
		if (states.isEmpty()) { return; }
		AbstractWallOverlayRenderState firstState = states.getFirst();
		BufferBuilder builder = ctx.start(() -> MaLiLibReference.MOD_ID+":walls/batch_draw", firstState.pipeline());
		...
		try (BuiltBuffer meshData = builder.endNullable())
		{
			if (meshData != null)
			{
				ctx.draw(meshData, false, setLineWidth);
			}
			ctx.reset();
		}
		catch (Exception e)
