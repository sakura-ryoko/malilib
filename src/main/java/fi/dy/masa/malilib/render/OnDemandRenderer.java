...
//	@ApiStatus.Internal
//	@Override
//	public void onExtractWorldLast(RenderTickCounter deltaTracker, Camera camera, float ticks, Profiler profiler)
//	{
//		this.onUpdateStates(camera, deltaTracker, profiler);
//	}

	@ApiStatus.Internal
	private void onUpdateStates(Camera camera, RenderTickCounter deltaTracker, Profiler profiler)
	{
		synchronized (this.rendererMap)
		{
	...
	@ApiStatus.Internal
	private void onUpdateEachState(String key, IOnDemandRenderer<?> renderer, Camera camera, RenderTickCounter deltaTracker, Profiler profiler)
	{
		this.lock.lock();
		try
		{
...
//				}
	                                      ...
	                                      Camera camera, BufferBuilderStorage buffers, Profiler profiler)
	{
		...
		CameraRenderState cameraRenderState = mc.gameRenderer.getEntityRenderStates().cameraRenderState;
		RenderTickCounter deltaTracker = ((IWorldRenderer) mc.worldRenderer).malilib_getDeltaTracker();
...
//	}
	...
	@ApiStatus.Internal
	private void onDrawStates(Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		synchronized (this.rendererMap)
		{
			Set<String> keys = this.rendererMap.keySet();
	...
	@ApiStatus.Internal
	private void onDrawEachState(String key, IOnDemandRenderer<?> renderer, Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		this.lock.lock();
		try
		{
			...
			if (state != null && renderer.shouldUseRenderContext())
			{
				...
				else
				{
					...
					try
					{
						ctx.drawPost(null, useColor, useLineWidth, useOffset, renderer.shouldUseLightmapTex());
					}
