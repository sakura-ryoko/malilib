...
public class CenterRangeWallOutlinesOverlayRenderState extends BoxWallOutlinesOverlayRenderState
{
	...
	private static Pair<BlockPos, BlockPos> calculatePositions(BlockPos posCenter, int chunkRange, World level)
	{
		final int cx = (posCenter.getX() >> 4);
		final int cz = (posCenter.getZ() >> 4);
		final int minY = level != null ? level.getBottomY() : -64;
		final int maxY = level != null ? level.getTopYInclusive() + 1 : 320;
	...
	public static CenterRangeWallOutlinesOverlayRenderState create(BlockPos posCenter, int chunkRange, World level, Vec3d camPos)
	{
