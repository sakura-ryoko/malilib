...
public class MaLiLibWorldTextRenderer implements Font.GlyphVisitor, AutoCloseable
{
	...
	public MaLiLibWorldTextRenderer()
	{
		...
		// , 0 -- 26.2
		this.context = new RenderContext(() -> MaLiLibReference.MOD_ID+"_world_text", RenderPipelines.TEXT);
...
//			RenderPipeline pipeline = this.grayscale ? this.selectGrayscale(this.mode) : this.selectColor(this.mode);
			RenderPipeline pipeline = this.selectColor(this.mode);

			// , 0 -- 26.2
			this.context.start(() -> MaLiLibReference.MOD_ID+"_text_plate", pipeline);
