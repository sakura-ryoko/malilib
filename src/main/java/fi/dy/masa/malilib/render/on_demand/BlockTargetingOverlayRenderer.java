	...
	private void checkConfigAndTarget(MinecraftClient mc)
	{
			...
			this.config.getBooleanValue())
		{
			...
			if (this.keybind != null)
			{
				...
				{
					this.scheduleTarget(entity.getHorizontalFacing(), hitResult.getBlockPos(), hitResult.getSide(), Vec3d.of(hitResult.getPos()));
				}
				else { this.reset(); }
			}
			...
			if (this.renderCenter != null && !this.renderCenter.isUploaded())
			{
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						...
						this.renderCenter.lineWidth(this.currentEntry.centerState().lineWidth());
						meshData.close();
					}
				}
				catch (Exception err)
			...
			if (this.renderEdges != null && !this.renderEdges.isUploaded())
			{
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						...
						this.renderEdges.lineWidth(this.currentEntry.edgesState().lineWidth());
						meshData.close();
					}
				}
				catch (Exception err)
