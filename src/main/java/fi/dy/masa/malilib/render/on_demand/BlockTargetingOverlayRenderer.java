...
public class BlockTargetingOverlayRenderer implements IOnDemandRenderer<AbstractBlockTargetingOverlayRenderState>
{
	...
	private void checkConfigAndTarget(MinecraftClient mc)
	{
			...
			this.config.getBooleanValue())
		{
			...
			if (this.keybind != null)
			{
				...
				{
					this.scheduleTarget(entity.getFacing(), hitResult.getBlockPos(), hitResult.getSide(), hitResult.getPos());
				}
				else { this.reset(); }
			}
	...
	private void scheduleTarget(net.minecraft.util.math.Direction facing, net.minecraft.util.math.BlockPos pos, net.minecraft.util.math.Direction side, net.minecraft.util.math.Vec3d hitVec)
	{
		...
		if (this.currentEntry != null)
		{
			...
			if (this.renderCenter != null && !this.renderCenter.isUploaded())
			{
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						this.renderCenter.lineWidth(this.lineWidth);
			...
			if (this.renderEdges != null && !this.renderEdges.isUploaded())
			{
				...
				try (BuiltBuffer meshData = buffer.endNullable())
				{
					if (meshData != null)
					{
						this.renderEdges.lineWidth(this.lineWidth);
	...
	public @Nullable AbstractBlockTargetingOverlayRenderState updatePre(Camera camera, RenderTickCounter tracker, Profiler profiler)
	{
		if (this.hasData() && this.dirty)
		{
			...
			{
				this.lastTarget = this.currentTarget;
				this.currentEntry = this.buildEntry(Vec3d.of(camera.getCameraPos()), this.currentTarget.facing(), this.currentTarget.pos(), this.currentTarget.side(), this.currentTarget.hitVec());
