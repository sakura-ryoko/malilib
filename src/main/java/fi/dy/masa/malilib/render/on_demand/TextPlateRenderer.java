...
public class TextPlateRenderer implements IOnDemandRenderer<AbstractTextPlateRenderState>
{
	...
	@Override
	public @Nullable AbstractTextPlateRenderState updatePre(Camera camera, RenderTickCounter tracker, Profiler profiler)
	{
		if (this.hasData())
		{
			this.tickDelta = tracker.getTickProgress(true);
	...
	@Override
	public @Nullable AbstractTextPlateRenderState drawPre(Matrix4fc modelViewMatrix, CameraRenderState cameraState, Profiler profiler)
	{
		if (this.hasCurrentData())
		{
...
//			catch (Exception ignored) {}
	...
	private void drawEachInternal(AbstractTextPlateRenderState state, CameraRenderState cameraState)
	{
		...
		double cx = cameraState.pos.getX();
		double cy = cameraState.pos.getY();
		double cz = cameraState.pos.getZ();
		float fYaw = camEntity.getYaw(this.tickDelta);
		float fPitch = camEntity.getPitch(this.tickDelta);
		...
		for (String line : st.text())
		{
			...
			final int lineWidth = font.getWidth(comp);
...
//			preparedText.visit(renderer);
			...
			font.draw(comp, textX, textY,
			                 st.seeThrough() ? (0x20000000 | (textColor & 0xFFFFFFFF)) : textColor,
			                 false, matrix4f, immediate,
			                 st.seeThrough() ? TextRenderer.TextLayerType.SEE_THROUGH : TextRenderer.TextLayerType.POLYGON_OFFSET,
			                 0, 15728880
			);
