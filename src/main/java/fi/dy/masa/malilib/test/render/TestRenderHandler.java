        ...
        if (MaLiLibConfigs.Test.TEST_CONFIG_BOOLEAN.getBooleanValue())
        {
            ...
            if (mc.player != null)
            {
                ...
                if (ConfigTestEnum.TEST_TEXT_PLATE.getBooleanValue())
                {
                    ...
                    {
                        TestTextPlateRenderer.INSTANCE.render(Vec3d.of(camera.getCameraPos()), mc, profiler);
                    }
                }
    ...
    public void onRenderTooltipLast(DrawContext drawContext, ItemStack stack, int x, int y)
    {
        ...
        else if (stack.isOf(Items.ENDER_CHEST))
        {
            if (MaLiLibConfigs.Test.TEST_CONFIG_BOOLEAN.getBooleanValue() && GuiBase.isShiftDown())
            {
                ...
                if (player != null)
                {
                    Pair<Entity, NbtCompound> pair = TestDataSyncer.INSTANCE.requestEntityNbt(world, player.getId());
