...
//        RenderUtils.polygonOffset(-1.2f, -0.2f);
        ...
        if (this.posLookingAt != null)
        {
            RenderUtils.renderBlockOutline(this.posLookingAt, expand, lineWidthBlockBox, this.colorLooking, mc);
        }
...
//        RenderUtils.depthMask(true);
    ...
    public void renderSelection(Matrix4f posMatrix, Matrix4f projMatrix, Profiler profiler, Selection selection, MinecraftClient mc)
    {
        ...
        if (pos1 != null && pos2 != null)
        {
            if (!pos1.equals(pos2))
            {
                RenderUtils.renderAreaOutlineNoCorners(pos1, pos2, lineWidthArea, this.colorX, this.colorY, this.colorZ, mc);
                RenderUtils.renderAreaSides(pos1, pos2, this.sideColor, posMatrix, mc);
                RenderUtils.renderBlockOutline(pos1, expand, lineWidthBlockBox, this.colorPos1, mc);
                RenderUtils.renderBlockOutline(pos2, expand, lineWidthBlockBox, this.colorPos2, mc);
            }
            else
            {
                RenderUtils.renderBlockOutlineOverlapping(pos1, expand, lineWidthBlockBox, this.colorPos1, this.colorPos2,
                                                          this.colorOverlapping, posMatrix, mc);
            }
        }
        else
        {
