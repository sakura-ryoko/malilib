...
public class WidgetTestBlockStateList extends WidgetListBase<GuiTestBlockStateList.Entry, WidgetTestBlockStateListEntry>
{
    ...
    private void buildAllEntries()
    {
        ...
        {
            this.entries.add(new GuiTestBlockStateList.Entry(block.getName().getString(), block.getDefaultState()));
        }
    }
    ...
    {
        return new WidgetTestBlockStateListEntry(x, y, this.browserEntryWidth,
                                                 Math.max(this.getBrowserEntryHeightFor(entry), this.parent.iconSize + 6),
