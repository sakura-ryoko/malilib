...
public class ConfigBlockState extends ConfigBase<ConfigBlockState> implements IConfigBlockState
{
	...
	{
		return BlockArgumentParser.stringifyBlockState(this.defaultValue);
	}
	...
	{
		return BlockArgumentParser.stringifyBlockState(this.value);
	}
			...
			if (!this.value.equals(oldValue) || this.isDirty())
			{
				...
				if (!Objects.equals(this.getLastBlockStateValue(), this.getBlockStateValue()))
				{
                    MaLiLib.LOGGER.error("[BLOCK_STATE/{}]: setValueFromJsonElement(): LV: [{}], OV: [{}], NV: [{}]", this.getName(),
                                         BlockArgumentParser.stringifyBlockState(this.getLastBlockStateValue()),
                                         BlockArgumentParser.stringifyBlockState(oldValue),
                                         BlockArgumentParser.stringifyBlockState(this.getBlockStateValue())
                    );
